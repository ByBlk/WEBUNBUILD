...
export const AlerteJob: React.FC<INotificationPresetProps> = ({ distance = 0, distancepnj = 0, ...props }) => {
  const [styleColor, setStyleColor] = useState("");

  const unit = useMemo(() => {
    if (distance < 1000) {
      const num = Number(distance);
      const roundedString = num.toFixed(0);
      setStyleColor("green");
      return roundedString + "m";
    }
    if (distance > 1000) {
      const num = Number(distance / 1000);
      const roundedString = num.toFixed(1);
      setStyleColor("yellow");
      return roundedString + "km";
    }
  }, [distance]);
  ...
  return <div className="notification police">
    ...
    <div className="main">
      <div className="line">
        <MediaCdn path={"assets/hud/notifications"} name={"info.svg"} props={{
          style: {
            animationDelay: ".8s"
          },
      ...
      <div className="line">
        <MediaCdn path={"assets/hud/notifications"} name={"police.svg"} props={{
          style: {
            animationDelay: ".8s"
          },
          className: " A-BounceIn"
    ...
    <div
      className={"distance " + styleColor + " A-BounceIn"}
